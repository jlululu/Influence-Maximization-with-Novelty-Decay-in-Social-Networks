...
public public class Test{
  public static double ND(int cnt, List<Double> Pcon_list, int curr, int con, double pro){
	if(cnt == curr)
	  return pro*Math.pow(0.3,con);
    double t = Pcon_list.get(curr);
	return ND(cnt,Pcon_list,curr+1,con+1,pro*t)+ND(cnt,Pcon_list,curr+1,con,pro*(1-t));
  }

  public static double PPND_Inf_IC(DiGraph G, Set<Node> S){
    int C = 5;
    ...
    while(!PH.isEmpty()){
      ...
      if(u.label != -1){
        ...
        inf = (v.weight)*pcon*ND(t,Pcon.getOrDefault(w,new ArrayList<Double>()),0,0,1.0);
      }
      if(inf >= theta){
        Count.put(w,t+1);
...
//   R-Greedy + PPND
  public static Set<Node> R_Greedy_IC(DiGraph G, int k, Set<Node> S_prev1, Set<Node> visited1, Map<Node,Double> Q1, Map<Node,Double> Q_prev1, TreeSet<Node> Q_list1){
    Map<Integer,Node> M = G.m;
  ...
  public static int IC(DiGraph G, Set<Node> S){
    ...
    for(int i = 0;i < 10000;i ++){
      ...
      double inf = 1.0;
      if(u.label != -1)
        inf = E.get(Arrays.asList(u,w)).weight*Math.pow(0.3,t);
      if(Math.random() <= inf){
        activated.add(w);
        sigma ++;
        for(int ind : w.successor){
...
//   R-Greedy + MC
  public static Set<Node> R_Greedy_IC1(DiGraph G, int k, Set<Node> S_prev1, Set<Node> visited1, Map<Node,Double> Q1, Map<Node,Double> Q_prev1, TreeSet<Node> Q_list1){
    Map<Integer,Node> M = G.m;
    Set<Node> S_prev = new HashSet<Node>();
    Set<Node> visited = new HashSet<Node>();
    Map<Node,Double> Q = new HashMap<Node,Double>();
...
// initialize for R_Greedy_IC
    ...
    for(Node v : V){
      ...
      s.add(v);
      double temp = PPND_Inf_IC(G,s);
      Q1.put(v,temp);
      visited1.add(v);
      Q_prev1.put(v,temp);
      Q_list1.add(v);
    }
// initialize for R_Greedy_IC1
    ...
    System.out.println("R_Greedy_IC:");
    for(int k = 10;k <= 100;){
      long start1 = System.currentTimeMillis();
      Set<Node> R_seed = R_Greedy_IC(G,k,S_prev1,visited1,Q1,Q_prev1,Q_list1);
      long elapsedTimeMillis1 = System.currentTimeMillis()-start1;
      double inf_R = IC(G,R_seed);
      System.out.println("time="+elapsedTimeMillis1/1000F+"seed number="+R_seed.size()+"inf="+inf_R);
      k += 10;
    }
    
    System.out.println("R_Greedy_IC1:");
    for(int k = 10;k <= 100;){
      long start2 = System.currentTimeMillis();
      Set<Node> R_seed = R_Greedy_IC1(G,k,S_prev2,visited2,Q2,Q_prev2,Q_list2);
